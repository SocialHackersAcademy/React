	...
	return (
		<div>
			<form onSubmit={submitHandler} className="searchBar">
                <input type="text" value={cityName} required placeholder="Search City" onChange={(e) => setCityName(e.target.value)} />
				<input type="submit" value="Search" />
			</form>
            {cityData && cityData.map((city, index) => { return <CityBox key={index} id={city.id}  data={city} removeCity={removeCity}/>})}
		</div>
	);
}
